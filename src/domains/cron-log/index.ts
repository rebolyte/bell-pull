...
import { type CronLogEntry, type CronLogRow, parseCronLogRow } from "./schema.ts";
...
const write = ({ db }: CronLogDeps) => (entry: CronLogEntry): ResultAsync<void, AppError> =>
...
const query =
  ({ db }: CronLogDeps) => (jobName?: string, limit = 50): ResultAsync<CronLogRow[], AppError> => {
